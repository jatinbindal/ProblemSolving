...
public class PathSum3 {
    ...
    public void traverse(TreeNode node,int sum,List<List<Integer>> res,   List<Integer> ans){
        if(node == null) return;
    ...
    public void sum(TreeNode node, int curr, int sum,List<List<Integer>> res,List<Integer> ans){
        if (node == null) return;
        curr+=node.val;
        ans.add(node.val);
